...
export async function fetchAppDetails(jwtToken: string, appId: string): Promise<AppDetailsFromConnect> {
  ...
  // 1. Fetch App Store Versions & Version Localizations
  try {
    const res = await fetchWithTimeout(`https://api.appstoreconnect.apple.com/v1/apps/${appId}/appStoreVersions?limit=5&include=appStoreVersionLocalizations`, jwtToken);
    if (res.data && res.data.length > 0) {
      const primaryVersion = res.data[0];
      version = primaryVersion.attributes?.versionString || '1.0.0';

      // Find included localizations or fetch separately
      const includedLocs = res.included?.filter((inc: any) => inc.type === 'appStoreVersionLocalizations') || [];
      if (includedLocs.length > 0) {
        ...
        // Fetch version localizations explicitly
        ...
        if (locRes?.data && locRes.data.length > 0) {
          ...
          // Attempt to fetch screenshot sets for the primary localization
          try {
            const locId = locRes.data[0].id;
            const ssSetsRes = await fetchWithTimeout(`https://api.appstoreconnect.apple.com/v1/appStoreVersionLocalizations/${locId}/appScreenshotSets?include=appScreenshots`, jwtToken).catch(() => null);
              ...
              for (const item of ssSetsRes.included) {
                if (item.type === 'appScreenshots' && item.attributes?.imageAsset) {
                  ...
                  screenshots.push({
                    ...
                    width: asset.width || 1290,
                    height: asset.height || 2796,
            ...
            console.warn('[Connect] Screenshot fetch skipped:', ssErr);
          }
        }
      }
    }
  } catch (err) {
    console.warn('[Connect] Error fetching App Store Versions:', err);
  }

  // 2. Fetch App Infos & App Info Localizations
  try {
    const res = await fetchWithTimeout(`https://api.appstoreconnect.apple.com/v1/apps/${appId}/appInfos?include=appInfoLocalizations,ageRatingDeclaration`, jwtToken);
    appInfos = res.data || [];
    if (appInfos.length > 0) {
      const attributes = appInfos[0].attributes || {};
      ageRating = attributes.ageRatingDeclaration?.rating;

      const includedInfoLocs = res.included?.filter((inc: any) => inc.type === 'appInfoLocalizations') || [];
    ...
    console.warn('[Connect] Error fetching App Infos:', err);
  }

  // 3. Fetch Recent Builds
  try {
    const buildsRes = await fetchWithTimeout(`https://api.appstoreconnect.apple.com/v1/builds?filter[app]=${appId}&limit=1&sort=-uploadedDate`, jwtToken).catch(() => null);
    ...
    console.warn('[Connect] Error fetching Builds:', err);
  }

  // 4. In App Purchases V2
  try {
    const res = await fetchWithTimeout(`https://api.appstoreconnect.apple.com/v1/apps/${appId}/inAppPurchasesV2?limit=50`, jwtToken).catch(() => null);
    inAppPurchases = res?.data || [];
  } catch (err) {
    console.warn('[Connect] Error fetching In-App Purchases:', err);
  }

  // 5. Subscription Groups
  try {
    const res = await fetchWithTimeout(`https://api.appstoreconnect.apple.com/v1/apps/${appId}/subscriptionGroups?limit=50`, jwtToken).catch(() => null);
    subscriptionGroups = res?.data || [];
  } catch (err) {
    console.warn('[Connect] Error fetching Subscription Groups:', err);
  }
