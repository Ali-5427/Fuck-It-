...
class AppStore {
  ...
  public deleteApp(appId: string) {
    ...
    if (this.user) {
      // Explicitly delete child records tied to this app to prevent orphaned rows
      insforge.database.from('findings').delete().eq('app_id', appId).then(({ error }) => {
        if (error) console.error('Error deleting findings from InsForge:', error);
      });
      ...
      insforge.database.from('uploads').delete().eq('app_id', appId).then(({ error }) => {
        if (error) console.error('Error deleting uploads from InsForge:', error);
      });
      insforge.database.from('reports').delete().eq('app_id', appId).then(({ error }) => {
        if (error) console.error('Error deleting reports from InsForge:', error);
      });
